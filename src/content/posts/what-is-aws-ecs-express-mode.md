...
If the reason you need to step beyond Express Mode is blue/green or canary releases, the [deployment strategies guide](/posts/deployment-strategies-explained/) walks through what those strategies actually look like in practice and when each is worth the extra infrastructure cost.

If Express Mode still cannot pull your image when the service first comes up, see the [CannotPullContainerError fixes for ECS](/posts/fix-cannotpullcontainererror-ecs/) guide — the same seven root causes apply even when Express Mode wires up the rest of the stack for you.
