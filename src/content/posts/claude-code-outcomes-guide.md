...
For the surrounding Claude Code surface, the [Claude Code Skills complete guide](/posts/claude-code-skills-complete-guide/) covers how to pack the rubric and the prompt into a reusable skill, the [Claude Code Dreaming guide](/posts/claude-code-dreaming-guide/) explains how memory files stay clean across the many runs an unattended routine generates, and the [fix Claude Code when it feels slow](/posts/claude-code-slow-fix/) guide is worth keeping nearby for the weeks where outcomes regress because the tool itself feels off.
