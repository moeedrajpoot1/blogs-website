...
| 36 | Deployment Strategies: Recreate, Rolling, Blue-Green, Canary, A/B | "deployment strategies" | ✅ Published |

Article 36 (Deployment Strategies Explained) shipped 2026-06-06 as the first cross-pillar piece bridging Cluster 5 (DevOps) with general backend engineering search demand. The head term "deployment strategies" is saturated (Spacelift, AWS, GitLab, Harness all ranking) so the article targets the "plain-English with diagrams" visual-learner intent — lower-volume but weaker competition. Captures spillover on the individual strategy long-tails (`blue green deployment`, `canary deployment`, `rolling deployment`, `recreate deployment`, `a/b testing deployment`, `kubernetes deployment strategies`) and the comparison long-tails (`blue green vs canary`, `deployment strategies comparison`). Inbound links from 5 sibling DevOps articles (ECS, ECS Express Mode, ECR, Copilot EOL, ECS pull error) — strengthens Cluster 5 internal link graph and is also the most LinkedIn-shareable piece on the site (course-material topic, every backend engineer encounters this), which doubles as off-domain top-of-funnel. STRATEGIC RATIONALE: opens a second growth lane outside the Claude/Agent SDK pillars where competition is thinner, gives the blog a defensible second pillar, and gives existing DevOps posts a sixth strong outbound link target.
